...
database_location = "..\\Eva - Dependencies\\OMSI.csv"
...
# Update function
def update():
    # These are the folders we want to crawl through to collect information
    omart = folderstats('\\\\omsmds000\\EB3\\OMSI\\OMART\\', ignore_hidden=True)
    omart_structure = folderstats('\\\\omsmds000\\EB3\\OMSI\\OMART STRUCTURE\\', ignore_hidden=True)

    #  We combine the two dataframes
    frames = [omart,omart_structure]
    omsi = pd.concat(frames)

    # We will only be keeping the files
    omsi = omsi[omsi['folder']==False]

    # We give each file an id
    omsi['id'] = np.arange(len(omsi)) 

    #  Save the file to the drive
    omsi.to_csv(database_location)