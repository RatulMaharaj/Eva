...
def update_data():
    ...
    return render_template(
        "update.html", modtime=modtime, version=version, folders=folders_str
    )
...
def combine():
    if request.method == "POST":
        ...
        try:
            ...
            merge_pdfs(
                names, output=os.path.join(app.config["UPLOAD_FOLDER"], "merged.pdf")
            )

            if password != "":
                print("The password is not blank")
                encrypt_file(
                    os.path.join(app.config["UPLOAD_FOLDER"], "merged.pdf"),
                    password,
                    mode="encrypt",
                )
                return send_file(
                    os.path.join(app.config["UPLOAD_FOLDER"], "merged_encrypted.pdf"),
                    as_attachment=True,
                )
            return send_file(
                os.path.join(app.config["UPLOAD_FOLDER"], "merged.pdf"),
